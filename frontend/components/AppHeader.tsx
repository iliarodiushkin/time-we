...
export default function AppHeader({ rightSlot, showSearch, onSearch, searchValue }: Props) {
  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 40, background: "white", borderBottom: `1px solid ${B}` }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 40px", height: 58, display: "flex", alignItems: "center", gap: 16 }}>
        ...
        {showSearch && (
          <div style={{ flex: 1, maxWidth: 480, margin: "0 20px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F8F7FF", border: `1px solid ${B}`, borderRadius: 12, padding: "8px 14px" }}>
              ...
              <input
                placeholder="Поиск мероприятий, мест, идей..."
                value={searchValue || ""}
              ...
              {searchValue && (
                <button onClick={() => onSearch?.("")}
                  style={{ background: "none", border: "none", cursor: "pointer", color: M, fontSize: 16, lineHeight: 1, padding: 0 }}>
                  ✕
                </button>
              )}
            </div>
          ...
          {NAV.map(item => {
            ...
            return (
              <button key={item.href} onClick={() => router.push(item.href)}
                ...
                {item.icon(active)}
                <span style={{ fontSize: 10, fontWeight: active ? 600 : 400, color: active ? P : "#B2BEC3", transition: "color 0.2s" }}>
                  {item.label}
                </span>
              </button>
            );
        ...
        {/* Правый слот (кнопки сброса, выхода и т.д.) */}
        {rightSlot && (
